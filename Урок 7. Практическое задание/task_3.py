...
def create_lst(m):
    return [random.randint(1, 10) for _ in range(2 * m + 1)]
...
print(f"Реализация через сортировку(медиана находится по индексу m): {gnome_sort(orig_lst, m)}")
...
Замеры:
Хотела еще для 1000 сделать для наглядности, но не смогла дождаться результата))
